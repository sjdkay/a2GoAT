void ParaPerp_Combiner_Scatt_V2(){
    ...
    for(Int_t i = 0; i < 10; i++){ // Energy
        for(Int_t j = 0; j < 5; j++){ // Theta
            sprintf(HistName, "Phip_%iMeVCM%i", 410+(i*20) , j+1);
            sprintf(NewHistName, "Phip_%iMeVCM%i_Para", 410+(i*20) , j+1);
    ...
    TFile *f1 = new TFile("/scratch/Mainz_Software/Data/GoAT_Output/GoAT_23_01_17/Perp/Physics_Total_Perp_34_14_5_18.root"); // Open latest Para file
    ...
    for(Int_t i = 0; i < 10; i++){ // Energy
        for(Int_t j = 0; j < 5; j++){ // Theta
            sprintf(HistName, "Phip_%iMeVCM%i", 410+(i*20) , j+1);
            sprintf(NewHistName, "Phip_%iMeVCM%i_Perp", 410+(i*20) , j+1);
            PhipPerp[i][j] = ((TH1F*)f1->Get(HistName));
            PhipPerp[i][j]->SetName(NewHistName);
        }
    }

    TFile f2("ParaPerp_S34_Combined.root", "RECREATE");
