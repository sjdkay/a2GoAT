...
void CxAsymm_V4_5ThetaBin() {
    ...
    TGraphErrors* CxEg[10][5]; // As fn of Angular bin for fixed energies
    double SinAmps[9][8][5]; // To store and get Cx amps
    double SinAmpErrs[9][8][5];// To store and get Cx amps
    Double_t Cx[9][8][5];
    Double_t CxErr[9][8][5];
    ...
    TFile Results_File("AsymmFits_PTotal_154_54_V4_TEST.root", "RECREATE");
    ...
    for(Int_t i = 0; i < 9; i++){ // Centre of energy bin
        for(Int_t k = 0; k < 5; k++)){ // CosThetaBin
            ...
            CxEg[i][k]->SetTitle(Form("C_{x'}(E_{#gamma}) #theta_{CM} %i - %i^{#circ}", k*36 ,36 + (k*36)));
    ...
    for(Int_t k = 0; k < 5; k++)){ // CosThetaBin
        ...
        CxEg[9][k]->SetTitle(Form("C_{x'}(E_{#gamma}) #theta_{CM} %i - %i^{#circ}", k*36 ,36 + (k*36)));
    ...
    C4->Divide(3, 2, 0.000000001, 0.01);
    for(Int_t k = 0; k < 5; k++){
        C4->cd(k+1);
        CxEg[9][k]->Draw("AEP");
    }
