...
void CxAsymm_V3() {
    ...
    TH1F* AsymmHists[3][8][5];
    TGraphErrors* CxPlots[5][8]; // As fn of Angular bin for fixed energies
    TGraphErrors* Cx2Plots[3][5]; // As fn of energy bin for fixed angular bins
    ...
    double SinAmps[3][8][5]; // To store and get Cx amps
    double SinAmpErrs[3][8][5];// To store and get Cx amps
    double CosAmps[8][5];
    double CosAmpErrs[8][5];
    double MeanX[8][5];
    double MeanY[8][5];

    Double_t Cx[3][8][5]; // Fixed P Chi2 fit, Fixed P LL fit, Variable p Chi2 fit, Variable P LL fit
    Double_t CxErr[3][8][5];
    Double_t py[8][5];
    Double_t pyErr[8][5];

    Double_t Cx2[5][8]; // Fixed P Chi2 fit, Fixed P LL fit, Variable p Chi2 fit, Variable P LL fit
    Double_t Cx2Err[5][8];
    ...
    Double_t AmpVal;
    Double_t AEff[8][5];
    Double_t CorrFac[8][5];
    Double_t AEffCorr[8][5];
    ...
    for(Int_t i = 0; i < 3; i++){
        for(Int_t j = 0; j < 8; j++){
            ...
            CxPlots[i][j] = new TGraphErrors(5 , x, Cx[i][j], ex, CxErr[i][j]);
...
double ex2[8] = {50, 50, 50, 50, 50, 50, 50, 50};
    ...
    leg2 = new TLegend(0.3, 0.3, 0.6, 0.6);
    ...
    for(Int_t j = 0; j < 8; j++){
        ...
        pyPlots[j] = new TGraphErrors(5 , x, py[j], ex, pyErr[j]);
    ...
    canvas5->Divide(3, 2, 0.000000001, 0.01);
