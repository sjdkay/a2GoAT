...
void CxAsymm_V4_3ThetaBin() {
    ...
    TProfile* PhiScAEffXProfile[2][8][3]; // Need each helicity so 0 is -ve, 1 is +ve for first index
    TH1D* PhiScAEffXProjection[2][8][3];
    TH1D* AsymmHists[3][8][3];
...
//    TFile *f = new TFile("/scratch/Mainz_Software/a2GoAT/Physics_Total_Amo147_Lin47_Combined_V2.root"); // Open the latest PTotal file to load histograms from
    ...
    TFile *f = new TFile("/d4tb1/sjdkay/MainzRecoil/GoAT_Output_Files/nobackup/GoAT_Files/Amo/Physics_Total_153_1_10_18.root"); // Open the latest PTotal file to load histograms from
    ...
    // Default version probably finem averages over all bins y
   ...
   TFile f1("AsymmFits_PTotal_153_53_V4_TEST.root", "RECREATE");
    ...
    for(Int_t j = 0; j < 8; j++){ // Energy
            for(Int_t k = 0; k < 3; k++)){ // CosThetaBin
                sprintf(PosHelHistName, "PhiScAEff%iPosHelCM%i", 250+(j*100), k+1);
                sprintf(NegHelHistName, "PhiScAEff%iNegHelCM%i", 250+(j*100), k+1);
                sprintf(PosHelProfName, "AEff%iPosHelCM%i", 250+(j*100), k+1);
                sprintf(NegHelProfName, "AEff%iNegHelCM%i", 250+(j*100), k+1);
                sprintf(PosHelProjName, "PhiScProj%iPosHelCM%i", 250+(j*100), k+1);
                sprintf(NegHelProjName, "PhiScProj%iPosHelCM%i", 250+(j*100), k+1);

                PhiScAEffXProfile[0][j][k] = ((TH2F*)f->Get(NegHelHistName))->ProfileX(NegHelProfName, 0, -1, "oe"));
                PhiScAEffXProfile[1][j][k] = (((TH2F*)f->Get(PosHelHistName))->ProfileX(PosHelProfName, 0, -1, "oe"));
                PhiScAEffXProjection[0][j][k] = (((TH2D*)f->Get(NegHelHistName))->ProjectionX(NegHelProjName, 0, -1, "oe"));
                PhiScAEffXProjection[1][j][k] = (((TH2D*)f->Get(PosHelHistName))->ProjectionX(PosHelProjName, 0, -1, "oe"));

                PhiScAEffXProfile[0][j][k]->Write();
                PhiScAEffXProfile[1][j][k]->Write();
                PhiScAEffXProjection[0][j][k]->Write();
                PhiScAEffXProjection[1][j][k]->Write();
            }
    }
