...
Bool_t	PPiPolTest::Start()
{
  ...
  cout << i << "  Events with forward nucleon, pion into CB" << endl;
  cout << k << "  Events with forward nucleon, pion into CB that is detected" << endl;
...
void	PPiPolTest::ProcessEvent()
{
  ...
  for (Int_t j = 0; j < GetTagger()->GetNTagged(); j++){
    ...
    if ((Nucleon1 == kTRUE) && (Pion1 == kTRUE)) return; // If 1st particle flagged as both drop out
    if ((Nucleon2 == kTRUE) && (Pion2 == kTRUE)) return;
    if ((Nucleon1 == kFALSE) && (Pion1 == kFALSE)) return; // If particle 1 not flagged as pion or nucleon drop out
    if ((Nucleon2 == kFALSE) && (Pion2 == kFALSE)) return;
    if ((Nucleon1 == kTRUE) && (MCTrueID1 == 14)) MN = Mp;
    if ((Nucleon1 == kTRUE) && (MCTrueID1 == 13)) MN = Mn;
    if ((Nucleon2 == kTRUE) && (MCTrueID1 == 14)) MN = Mp;
    if ((Nucleon2 == kTRUE) && (MCTrueID1 == 13)) MN = Mn;
    if ((Pion1 == kTRUE) && (NeutralPion == kFALSE)) MPi = MpiC;
    if ((Pion1 == kTRUE) && (NeutralPion == kTRUE)) MPi = Mpi0;
    if ((Pion2 == kTRUE) && (NeutralPion == kFALSE)) MPi = MpiC;
    if ((Pion2 == kTRUE) && (NeutralPion == kTRUE)) MPi = Mpi0;
    GetTrueTheta(); // Get true theta in each event
    if ((ThetaNTrue == 1000 ) || (ThetaPiTrue == 1000)) return; // Theta is set to 1000 if the event does not feature a nucleon or a nion, shouldn't occur
    if (ThetaNTrue > 20) return; // We only want to select events where the nucleon not going into CB
    if ( (ThetaPiTrue < 20) || (ThetaPiTrue > 160)) return; // Want pion to be going into CB
    i++; // Add one to our counter, this is the number of events where the nucleon went forward and the pion went into CB
    ...
    // If our pion is going into CB expect it to produce a track
    // Do NOT expect nucleon to give a track is it is going into TAPS which is switched off in the simulation currently


    if (NeutralPion == kFALSE){
        if (NTrack != 1) return; //In case with charged pion only want to detect 1 track, the charged Pion
        GetDetectorHitInfoChargedPi(); // Get # of hits in various detectors, expect a hit in all for a pion
        if ((CBHits1 == 0 || (PIDHits1 == 0)) || (MWPCHits1 == 0)) return; // If any of the detectors we expect a hit in does not detect a hit drop out
        GetTrackVector();
        GetTrackInfo(); // Get info for the track
        MCSmearing(); // Smear dE values
    }

    if (NeutralPion == kTRUE){
        if (NPi0 != 1) return; // If event had a simulated Pi0 drop out if we don't detect it
        cout << NTrack  << "  " << NPhotons << "   " << NRoo << endl;
        GetDetectorHitInfoNeutralPi();
        GetTrackVectorPi0(); // Will need different ffunctions for Pi0 case?
        GetTrackInfoPi0();
        MCSmearingPi0();
    }
...
Int_t PPiPolTest::GetEvent() // Gets basic info on particles for event
{
  ...
  NPi0 = GetNeutralPions()->GetNParticles();
...
Double_t PPiPolTest::GetTrueTheta(){
    ...
    EPiTrue = (PionTrueVect(3)*1000)- MPi;
    TrueGamma = GetGeant()->GetBeam();
    EgTrue = TrueGamma(3)*1000;
    }

    else if ((Nucleon2 == kTRUE) && (Pion1 == kTRUE)){
    ...
    PionTrue3Vect = PionTrueVect.Vect();
    ThetaPiTrue = PionTrue3Vect.Theta() * TMath::RadToDeg();
    EPiTrue = (PionTrueVect(3)*1000)- MPi; // In this case the value in brackets is the component of the vector!
    TrueGamma = GetGeant()->GetBeam();
    EgTrue = TrueGamma(3)*1000;
    }

    else if (((Nucleon1 == kFALSE) && (Pion2 == kFALSE)) || ((Nucleon2 == kFALSE) && (Pion1 == kFALSE)) )ThetaNTrue = 1000, ThetaPiTrue = 1000, EPiTrue = 0, ENTrue = 0;

    return ThetaNTrue, ThetaPiTrue, EPiTrue, ENTrue, EgTrue;
}

Int_t PPiPolTest::GetDetectorHitInfoChargedPi(){

    PIDHits1 = GetDetectorHits()->GetPIDHits(0);
    MWPCHits1 = GetDetectorHits()->GetMWPCHits(0);
    CBHits1 = GetDetectorHits()->GetNaIHits(0);

    return PIDHits1, MWPCHits1, CBHits1;
}

Int_t PPiPolTest::GetDetectorHitInfoNeutralPi(){
    ...
    CBHits2 = GetDetectorHits()->GetNaIHits(1);

    return PIDHits1, MWPCHits1, CBHits1, PIDHits2, MWPCHits2, CBHits2;
}
...
TLorentzVector PPiPolTest::GetTrackVectorPi0(){
...
Double_t PPiPolTest::GetTrackInfoPi0(){
...
Double_t PPiPolTest::MCSmearingPi0() // Smear dE values for MC data to represent Energy resolution of PID
{
...
PPiPolTest::PPiPolTest() // Define a load of histograms to fill
{
  ...
  ThetaPiEPiTrue = new GH2("ThetaPiEPiTrue", "True Theta vs Energy for Pions", 90, 0, 180, 200, 0, 1600);
  ThetaPiEPi = new GH2("ThetaPiEPi", "Theta vs Energy for Pions", 90, 0, 180, 200, 0, 800);
  EPiEgTrue = new GH2("EPiEgTrue", "True Pion Energy vs True Photon Energy", 200, 0, 1600, 200, 0, 1600);
  EPiEg = new GH2("EPiEg", "Pion Energy vs Photon Energy", 200, 0, 400, 200, 0, 1600);
  ...
  EgThetaEPiTrue = new GH3("EgThetaEPiTrue", "True Pion Energy as a Fn of Eg and True Theta", 400, 0, 1600, 60, 0, 180, 200, 0, 1600 );
