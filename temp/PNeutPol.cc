...
void	PNeutPol::ProcessEvent()
{
  ...
  for (Int_t j = 0; j < GetTagger()->GetNTagged(); j++){
    ...
    if(nBanana == kTRUE) continue; // Ones in banana are from scattering in cell, don't want to analyse these so drop out
    //if (mmp > 950) continue; //Test cut to look at events with low proton MM
    if(Zn == 0) continue; // if Zn is at exactly 0 drop the event
    ...
    if (nBanana == kFALSE && (zWCRec-zWC > 20 || zWCRec-zWC < -20)) continue; // Check tracks roughly match up to meet at same point in WC
  ...
  if(ProtonParticleNumber == 1)
    {
      ...
      Proton1 = kTRUE;
      Proton2 = kFALSE;
      MWPC_E0_n = GetTracks()->GetMWPC0Energy(1);
      MWPC_E1_n = GetTracks()->GetMWPC1Energy(1);
    }
  ...
  if(ProtonParticleNumber == 2)
    {
      ...
      Proton1 = kFALSE;
      Proton2 = kTRUE;
      MWPC_E0_n = GetTracks()->GetMWPC0Energy(0);
      MWPC_E1_n = GetTracks()->GetMWPC1Energy(0);
...
PNeutPol::PNeutPol() // Define a load of histograms to fill
{
  ...
  MWPCEnergy0_n = new GH1 ("MWPCE0Neutron", "Neutron MWPCE0 Distribution", 150, 0, 300);
  MWPCEnergy1_n = new GH1 ("MWPCE1Neutron", "Neutron MWPCE1 Distribution", 150, 0, 300);
...
void PNeutPol::FillHists()
{
  ...
  MWPCEnergy0_n->Fill(MWPC_E0_n, TaggerTime);
  MWPCEnergy1_n->Fill(MWPC_E1_n, TaggerTime);
